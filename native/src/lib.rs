...
pub fn deno_plugin_init(interface: &mut dyn Interface) {
    ...
    interface.register_op("op_serial_write_data_terminal_ready", op_serial_write_data_terminal_ready);
    interface.register_op("op_serial_write_request_to_send", op_serial_write_request_to_send);
    ...
    interface.register_op("op_serial_read_data_set_ready", op_serial_read_data_set_ready);
    interface.register_op("op_serial_read_ring_indicator", op_serial_read_ring_indicator);
    interface.register_op("op_serial_read_carrier_detect", op_serial_read_carrier_detect);
...
fn op_available_ports(_interface: &mut dyn Interface, _args: &mut [ZeroCopyBuf]) -> Op {
    ...
    for port in ports {
        ...
        match port.port_type {
            SerialPortType::BluetoothPort => { port_type = 3; }
            SerialPortType::PciPort => { port_type = 1; }
            ...
            SerialPortType::Unknown => { port_type = 4; }
        }
...
fn op_new_serial(_interface: &mut dyn Interface, args: &mut [ZeroCopyBuf]) -> Op {
    ...
    let port = serialport::new(path, baud_rate).open().expect("failed to open serialport");
...
fn op_serial_write_data_terminal_ready(_interface: &mut dyn Interface, args: &mut [ZeroCopyBuf]) -> Op {
    let id = u32::from_str(std::str::from_utf8(args.get(0).unwrap()).unwrap()).unwrap();
    let level = u32::from_str(std::str::from_utf8(args.get(1).unwrap()).unwrap()).unwrap() == 1;
    PORTS.with(|map| {
        if let Some(target) = map.borrow_mut().get_mut(&id) {
...
fn op_serial_bytes_to_read(_interface: &mut dyn Interface, args: &mut [ZeroCopyBuf]) -> Op {
    ...
    PORTS.with(|map| {
        if let Some(target) = map.borrow_mut().get_mut(&id) {
            let res = target.bytes_to_read().unwrap().to_string().as_bytes().to_vec().into_boxed_slice();
            Op::Sync(res)
...
fn op_serial_bytes_to_write(_interface: &mut dyn Interface, args: &mut [ZeroCopyBuf]) -> Op {
    ...
    PORTS.with(|map| {
        if let Some(target) = map.borrow_mut().get_mut(&id) {
            let res = target.bytes_to_write().unwrap().to_string().as_bytes().to_vec().into_boxed_slice();
            Op::Sync(res)
        } else { let res= b"n"; Op::Sync(res.to_vec().into_boxed_slice()) }
    })
}
...
fn op_serial_write_all(_interface: &mut dyn Interface, args: &mut [ZeroCopyBuf]) -> Op {
    ...
    PORTS.with(|map| {
        if let Some(target) = map.borrow_mut().get_mut(&id) {
            ...
            let res = b"1";
            Op::Sync(res.to_vec().into_boxed_slice())
        } else { let res= b"0"; Op::Sync(res.to_vec().into_boxed_slice()) }
    })
}
...
fn op_serial_clear(_interface: &mut dyn Interface, args: &mut [ZeroCopyBuf]) -> Op {
    ...
    let buffer = if buf == 0 { ClearBuffer::Input } else if buf == 1 { ClearBuffer::Output } else { ClearBuffer::All };
    PORTS.with(|map| {
        if let Some(target) = map.borrow_mut().get_mut(&id) {
...
fn op_serial_read_clear_to_send(_interface: &mut dyn Interface, args: &mut [ZeroCopyBuf]) -> Op {
    ...
    PORTS.with(|map| {
        if let Some(target) = map.borrow_mut().get_mut(&id) {
            ...
            if r.is_err() { res = b"0"; }
            else { res = if r.unwrap() == true { b"0" } else { b"1" }; }
            Op::Sync(res.to_vec().into_boxed_slice())
        } else { let res= b"1"; Op::Sync(res.to_vec().into_boxed_slice()) }
    })
}
